...
import { MoreVert, PhoneDisabled, ThumbUpAlt, VideocamOff } from '@material-ui/icons';
...
function ChatMessage({message,own}) {
    ...
    return (
        ...
        <div className={clsx(classes.chatMessage, own? `${classes.chatMessageRight}`: '')} >
            <Avatar 
            ...
            <div>
                {text &&
                    <div className={clsx(classes.textWrapper, own? `${classes.textWrapperColor}`: '')} >
                        <Typography 
                            className={clsx(classes.textContent, own? `${classes.flexFirstRight}`: '')} 
                            color="textPrimary"
                            component="p" 
                            variant="body"
                            >{text}
                        </Typography>
                        <Typography component="span" variant="caption"  color="textSecondary" className={classes.times} >{moment(createdAt).fromNow()}</Typography>
                ...
                }
                {
                    media?.map((item,index)=> (
                        <div key={index} className={clsx(classes.media, own? `${classes.mediaRight}`: '')}>
                            {
                                item.type.match(/video/i) && <video controls src={item.url} alt="video" width="25%" />
                            }
                            {
                            ...
                            {
                                item.type.match(/application/i) && <a href={item.url} target="_blank" rel="noreferrer">file ne</a>
                            }

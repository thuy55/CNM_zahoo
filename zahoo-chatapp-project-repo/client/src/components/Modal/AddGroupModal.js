import { Avatar, Button, Chip, Fade, InputAdornment, List, ListItem, ListItemAvatar, ListItemText, Paper, TextField, Typography } from '@material-ui/core'
...
function AddGroupModal() {
    ...
    const handleSubmitForm = React.useCallback((values) => {
        ...
        dispatch(postConversation(data,token))
        setListMember([])
        // setLabel('')
        handleHideModal()
   ...
   const handleDeleteMember = (item) => {
                ...
                <Paper className={classes.paper} id="modal-add-group">
                    ...
                    >
                            ...
                            {listFriend &&  <Typography>Ban be ({listFriend.length})</Typography>}
                                <List style={{height:"400px",overflowY:"scroll"}}>
                                    {listFriend?.map((item,index) => (
                                        <ListItem button onClick={() => handleAddMember(item)} key ={index}>
                                            <ListItemAvatar>
                                                <Avatar alt="avatar" />
                                            </ListItemAvatar>
                        ...
                        <div className={classes.actions}>
                            ...
                            <Button color="primary" variant="contained" type="submit" isSubmitting={isSubmitting}>Tạo nhóm</Button>
                        </div>
                        </Form>
                     )}
                     </Formik>
    ...
    return (
        <BaseModal body={body} isShow= {isShowAddGroupModal} />
    )
}

...
export const getImageAndVideo = (conversationId,token) => async dispatch => {
    const res = await getDataAPI(`conversations/get-image-and-video/${conversationId}`,token)
    try {
        dispatch({ 
...
export const getFiles = (conversationId,token) => async dispatch => {
    const res = await getDataAPI(`conversations/get-files/${conversationId}`,token)
...
//  change group name
export const change_Current_Conversation_GroupName = (conversation,newLabel,auth, socket) => async dispatch => {
...
//  out group
export const outGroup = (user, token, currentConversation, socket) => async dispatch => {
    ...
    try {
    const res = await postDataAPI(`conversations/out-group/${_id}`,data,token)
...
export const deleteGroup = (user, conversation, token, socket) => async dispatch => {
   
    const data={ userId: user._id}

    
    try {
    await postDataAPI(`conversations/delete-group/${conversation._id}`,data,token)
...
export const addMembersToGroup = (member, conversation,user, token, socket) => async dispatch => {
    //loc lay ra id
    
  const  _member = member.map(item => item._id)
    try {
       const res= await postDataAPI(`conversations/add-member-group/${conversation._id}`,{member:_member},token)
        dispatch({ 
